...
class TradingCalendarDispatcher(object):
    ...
    def get_calendar(self, name, start=None, end=None):
        ...
        # 通过交易日历的工厂类获取交易日历，交易日历工厂字典绑定是的交易日历的执行方法
        try:
        ...
        if start is not None and end is not None:
            calendar = self._calendars[canonical_name] = factory(start=start,
                                                                 end=end)
            return calendar
    ...
    def get_calendar_names(self):
        ...
        """
        # 返回所有交易日历的名字，包括别名，工厂类名字和日历本身的名字
        return list(
            set(
                itertools.chain(
