...
from .utils.data import get_cached
from pandas import Timestamp, date_range, DatetimeIndex
import pytz
from .utils.memoize import remember_last, lazyval
import warnings
...
class SHSZExchangeCalendar(TradingCalendar):
    ...
    由于中国股市存在午休，所以需要特殊处理日历
    Sample Code in ipython:
    > from zipline.utils.calendars import *
    > from zipline.yq.exchange_calendar_shsz import SHSZExchangeCalendar
    ...
    def __init__(self, start=start_default, end=end_default):
        with warnings.catch_warnings():
            ...
            result = mdb['shares_base_data']['trading_dates'].find(
                {'exchange': 'SHSE'},
                {
                    '_id': False,
                    'exchange': False,
            ...
            for date in result:
                datetime_list.append(date['day'])

            _all_days = DatetimeIndex(data=datetime_list, dtype='datetime64[ns, UTC]')
        ...
        TradingCalendar.__init__(self, start=start_default, end=end_default,all_days=_all_days)
